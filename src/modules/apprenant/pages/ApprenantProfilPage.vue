...
async function onChangePassword() {
  ...
  if (newPassword.value !== confirmPassword.value) {
    showToast('La confirmation ne correspond pas au nouveau mot de passe','info')
    return
  }
  ...
  try {
    ...
    const msg =
      error?.response?.data?.message || 'Échec de la modification du mot de passe'
    showToast(msg,'error')
  } finally {
...
const heroName = computed(() => `${profil.value.prenom} ${profil.value.nom}`.trim())
...
<template>
  <ApprenantLayout title="Mon profil" active-menu="profil">
    <section class="mx-auto max-w-3xl space-y-5">
      <div v-if="isLoading" class="rounded-xl border border-slate-200 bg-white p-5 text-sm text-slate-500 shadow-sm">
      ...
      <template v-else>
          ...
          <div class="relative flex flex-col items-start gap-5 sm:flex-row sm:items-center">
            ...
            <div class="flex-1">
              <div class="mb-2 inline-flex items-center gap-2 rounded-full bg-white/20 px-3 py-1">
                <span class="h-1.5 w-1.5 rounded-full" :class="isActif ? 'bg-green-400' : 'bg-red-400'"></span>
                <span class="text-xs font-semibold text-white">{{ isActif ? 'Apprenant actif' : 'Compte inactif' }}</span>
              </div>
              <h2 class="text-2xl font-extrabold leading-tight sm:text-3xl">{{ heroName || 'Profil apprenant' }}</h2>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div class="flex items-start justify-between border-b border-slate-100 px-6 py-5">
            <div>
              ...
              <p class="mt-0.5 text-sm text-slate-500">Mettez à jour les champs autorisés</p>
            </div>
          ...
          <div class="space-y-5 px-6 py-6">
            <div class="grid gap-4 sm:grid-cols-2">
              <div>
                <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">Prénom</label>
                <input v-model="profil.prenom" type="text" disabled class="w-full cursor-not-allowed rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-medium text-slate-600" />
              </div>
              <div>
                <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">Nom</label>
                <input v-model="profil.nom" type="text" disabled class="w-full cursor-not-allowed rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-medium text-slate-600" />
              </div>
            </div>

            <div>
              <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">Adresse email</label>
              <input v-model="profil.email" type="email" disabled class="w-full cursor-not-allowed rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-medium text-slate-600" />
            </div>

            <div class="grid gap-4 sm:grid-cols-2">
              <div>
                <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">Téléphone</label>
                <input
                  v-model="profil.telephone"
                  type="tel"
                  :disabled="!isEditing"
                  class="w-full rounded-xl border px-4 py-2.5 text-sm font-medium outline-none"
                  :class="isEditing ? 'border-orange-300 bg-white focus:border-orange-500' : 'cursor-not-allowed border-slate-200 bg-slate-50 text-slate-600'"
                />
              </div>
              <div>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">Adresse</label>
                <input
            ...
            <div class="grid gap-4 sm:grid-cols-2">
              <div>
                <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">Promotion</label>
                <input v-model="profil.promotion" type="text" disabled class="w-full cursor-not-allowed rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-medium text-slate-600" />
              </div>
              <div>
                <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">Référentiel</label>
                <input v-model="profil.referentiel" type="text" disabled class="w-full cursor-not-allowed rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-medium text-slate-600" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white px-6 py-6 shadow-sm">
          <h2 class="mb-5 text-base font-bold text-slate-900">Informations de formation</h2>
          <div class="grid gap-4 sm:grid-cols-2">
            <div class="rounded-2xl border border-slate-100 bg-slate-50/60 p-4">
              <p class="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Promotion</p>
              <p class="mt-1 text-sm font-bold text-slate-900">{{ profil.promotion }}</p>
            </div>
            <div class="rounded-2xl border border-slate-100 bg-slate-50/60 p-4">
              <p class="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Référentiel</p>
              <p class="mt-1 text-sm font-bold text-slate-900">{{ profil.referentiel }}</p>
            </div>
            <div class="rounded-2xl border border-slate-100 bg-slate-50/60 p-4">
              <p class="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Statut</p>
              <p class="mt-1 text-sm font-bold" :class="isActif ? 'text-emerald-700' : 'text-red-600'">
                {{ isActif ? 'Apprenant actif' : 'Compte inactif' }}
              </p>
            </div>
            <div class="rounded-2xl border border-slate-100 bg-slate-50/60 p-4">
              <p class="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Date d'inscription</p>
              <p class="mt-1 text-sm font-bold text-slate-900">{{ dateInscription }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-6 py-5 shadow-sm">
          <div class="flex items-center gap-3">
            <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-orange-50">
              <svg class="h-4 w-4 text-orange-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
            ...
            <div>
              ...
              <p class="text-xs text-slate-500">
                ...
                <span class="font-medium text-slate-700">{{ lastPasswordChange }}</span>
              </p>
            </div>
          </div>
          <button
    ...
    <Teleport to="body">
      <Transition name="modal">
        ...
        >
          <!-- backdrop -->
          <div class="absolute inset-0 bg-slate-900/50 backdrop-blur-sm" @click="closePasswordModal"></div>
          ...
          <div class="relative z-10 w-full max-w-md rounded-2xl bg-white shadow-2xl">
            ...
            <div class="flex items-center justify-between border-b border-slate-100 px-6 py-5">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <h3 class="text-base font-bold text-slate-900">Changer le mot de passe</h3>
                  <p class="text-xs text-slate-500">Remplissez les champs ci-dessous</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <div class="space-y-4 px-6 py-6">
              ...
              <!-- Mot de passe actuel -->
              <div>
                <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">Mot de passe actuel</label>
                <div class="relative">
                  <input
                  ...
                  <button type="button" tabindex="-1"
                    ...
                    <svg v-if="!showCurrentPassword" class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              ...
              <div>
                <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">Nouveau mot de passe</label>
                <div class="relative">
                  <input
                    v-model="newPassword"
                  ...
                  <button type="button" tabindex="-1"
                    ...
                    <svg v-if="!showNewPassword" class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                      <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/>
                    </svg>
                    <svg v-else class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                      <path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/>
              ...
              <!-- Confirmer -->
              <div>
                <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">Confirmer le nouveau mot de passe</label>
                <div class="relative">
                  <input
                    ...
                    :class="confirmPassword && newPassword
                  ...
                  <button type="button" tabindex="-1"
                    ...
                    <svg v-if="!showConfirmPassword" class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                      <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/>
                    </svg>
                    <svg v-else class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                      <path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/>
                ...
                <p v-if="confirmPassword && newPassword && confirmPassword !== newPassword" class="mt-1.5 text-xs text-red-500">
                  Les mots de passe ne correspondent pas
                </p>
                <p v-if="confirmPassword && newPassword && confirmPassword === newPassword" class="mt-1.5 text-xs text-emerald-600">
            ...
            <div class="flex gap-3 border-t border-slate-100 px-6 py-4">
              ...
              >
                <span v-if="isChangingPassword" class="flex items-center justify-center gap-2">
