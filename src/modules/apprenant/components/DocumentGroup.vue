...
<template>
  <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
    <div class="flex items-start gap-3">
      <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl" :class="typeIconBg(props.type)">
        <svg v-if="props.type === 'Stage'" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <rect x="2" y="7" width="20" height="14" rx="2" /><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2" />
        </svg>
        <svg v-else-if="props.type === 'Emploi'" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <rect x="2" y="7" width="20" height="14" rx="2" /><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2" />
        </svg>
        <svg v-else-if="props.type === 'Alternance'" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex flex-wrap items-center gap-2">
          <p v-if="props.entreprise" class="text-base font-bold text-slate-900">{{ props.entreprise }}</p>
          <span
        ...
        <p class="mt-0.5 text-sm text-slate-500">
          {{ props.type }} • Du {{ props.dateDebut }}<span v-if="props.dateFin"> au {{ props.dateFin }}</span>
        </p>
    ...
    <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
      <DocumentCard
        ...
        :type="doc.type"
        :date="doc.date"
        :is-deleting="props.deletingDocumentId === doc.id"
        @view="emit('viewDocument', $event)"
        @download="emit('downloadDocument', $event)"
