...
type MenuKey = 'dashboard' | 'validations' | 'apprenants' | 'import-historique' | 'statistiques' | 'entreprises'
...
const menu = computed(() => [
  ...
  { key: 'import-historique' as const, label: 'Import historique', to: '/import-historique' },
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-5">
          ...
          <div class="space-y-1">
            <RouterLink v-for="item in menu" :key="item.key" :to="item.to" :class="linkClass(item.key)">
              ...
              <svg v-else-if="item.key === 'validations'" class="h-[18px] w-[18px] shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              ...
              <svg v-else-if="item.key === 'entreprises'" class="h-[18px] w-[18px] shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <rect x="3" y="7" width="18" height="14" rx="2" /><path d="M8 7V5a4 4 0 0 1 8 0v2" />
              </svg>
              <svg v-else-if="item.key === 'import-historique'" class="h-[18px] w-[18px] shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
