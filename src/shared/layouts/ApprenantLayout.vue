...
const itemClass = (key: ApprenantMenuKey) => {
  const base = 'flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition-all duration-200'
...
<template>
  <div class="min-h-screen bg-slate-50 text-slate-900">
    ...
    <Transition name="slide-sidebar">
      <aside
        v-show="mobileOpen"
        class="fixed inset-y-0 left-0 z-50 flex w-72 flex-col bg-slate-900 shadow-2xl lg:hidden"
      >
        ...
        <nav class="flex-1 space-y-1 overflow-y-auto px-3 py-5">
          <p class="mb-3 px-4 text-[10px] font-semibold uppercase tracking-widest text-slate-600">Navigation</p>
          <RouterLink
            v-for="item in menu"
            :key="item.key"
            :to="item.to"
    ...
    <div class="flex min-h-screen flex-col lg:pl-64">
      ...
      <header class="sticky top-0 z-20 flex items-center justify-between border-b border-slate-200 bg-white/90 px-5 py-3.5 shadow-sm backdrop-blur-md lg:px-7">
        ...
        <div class="flex items-center gap-2 sm:gap-3">
          ...
          <!-- Avatar -->
          <div class="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-orange-400 to-orange-600 text-sm font-bold text-white shadow-md shadow-orange-500/30">{{ avatarInitial }}</div>
        </div>
      </header>
